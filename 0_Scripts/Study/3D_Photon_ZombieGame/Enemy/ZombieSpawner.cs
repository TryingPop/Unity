...
public class ZombieSpawner : MonoBehaviourPun, IPunObservable
{
    ...
    private void Awake()
    {
        ...
        PhotonPeer.RegisterType(typeof(Color), 128, 
            ColorSerialization.SerializeColor, ColorSerialization.DeserializeColor);        // Color 데이터를 전달하기 위한 방법
                                                                                            // ColorSerialization은 직접 정의한 클래스
    }
    ...
    // 포톤의 Network.Destroy()는 지연 파괴를 지원하지 않으므로 지연파괴를 직접 구현함
