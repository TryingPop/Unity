...
public class StageObject_Switch : MonoBehaviour
{
    ...
    // 외부 파라미터
    [HideInInspector] public int switchTurnCount = 0;
    ...
    void Awake()
    {
        ...
        switchTurnTime = 0.0f;
        switchinstantiateObjectsList =
            new GameObject[switchInstantiateObjects.Length];
        ...
        else
        {
            ...
            foreach (GameObject switchOnObject in switchOnObjects)
            {
                ...
                if (switchOnObject)
                {
                    ...
                    switchOnObject.SetActive(!switchOnObject.activeSelf);
                }
            }
            foreach(GameObject switchOffObject in switchOffObjects)
            {
                ...
                if (switchOffObject)
                {
                    ...
                    switchOffObject.SetActive(!switchOffObject.activeSelf);
                }
            }
        }
        ...
        for (int i = 0; i < switchInstantiateObjects.Length; i++)
        {
            ...
            if (switchInstantiateObjects[i])
            {
                ...
                if (sw)
                {
                    ...
                    switchinstantiateObjectsList[i] =
                        Instantiate(switchInstantiateObjects[i]) as GameObject;
                }
                else
                {
