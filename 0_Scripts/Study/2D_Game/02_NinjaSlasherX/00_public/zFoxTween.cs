        ...
        foreach (TweenItem tw in tweenItemList)
        {
            ...
            switch (tw.valueType) 
            {
                ...
                case zFOXTWEEN_VALUE.LOCALROTATION_X:
                    localRotation_x = TweenFloat(tw, localRotation_x, orgLocalRotationEA.x); break;
                case zFOXTWEEN_VALUE.LOCALROTATION_Y:
                    localRotation_y = TweenFloat(tw, localRotation_y, orgLocalRotationEA.y); break;
                case zFOXTWEEN_VALUE.LOCALROTATION_Z:
                    localRotation_z = TweenFloat(tw, localRotation_z, orgLocalRotationEA.z); break;
                case zFOXTWEEN_VALUE.LOCALROTATION:
                    localRotation_x = TweenFloat(tw, localRotation_x, orgLocalRotationEA.x);
                    localRotation_y = TweenFloat(tw, localRotation_y, orgLocalRotationEA.y);
                    localRotation_z = TweenFloat(tw, localRotation_z, orgLocalRotationEA.z);
                    break;
            ...
            transform.localRotation = Quaternion.Euler(
                localRotation_x, localRotation_y, localRotation_z);
            transform.localScale =
                new Vector3(localScale_x, localScale_y, localScale_z);
            if (sprite != null)
            {
    ...
    float TweenFloat(TweenItem tw, float nowN, float orgN)
    {
        ...
        switch (tw.opmMode)
        {
            ...
            case zFOXTWEEN_OPM.RANDOM:
                n = UnityEngine.Random.Range(tw.va, tw.vb); break;
        }
